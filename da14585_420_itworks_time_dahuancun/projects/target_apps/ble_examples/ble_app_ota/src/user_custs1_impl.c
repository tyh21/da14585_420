...
//    // 1. 将当前的 Unix 时间戳，转换为我们需要的 tm_t 结构体
//    //    这个 g_tm 变量将在后面的 sprintf 中被使用
//    transformTime(current_unix_time, &g_tm);
...
//    // 4. 在第 10 行，写公历日期
//    sprintf(temp_buf, "%d-%02d-%02d", g_tm.tm_year + YEAR0, g_tm.tm_mon + 1, g_tm.tm_mday);
//    EPD_DrawUTF8(10, 10, 1, temp_buf, EPD_ASCII_11X16, NULL, BLACK, WHITE);

//    // 5. 在第 50 行，写时间
//    sprintf(temp_buf, "%02d:%02d", g_tm.tm_hour, g_tm.tm_min);
//    EPD_DrawUTF8(10, 50, 1, temp_buf, EPD_ASCII_11X16, NULL, BLACK, WHITE);

//    // 6. 在第 260 行，写农历和星期
//    sprintf(temp_buf, "%s%s  星期%s", 
//            Lunar_MonthString[lunar_date.Month], 
//            Lunar_DateString[lunar_date.Date],
//            WEEKCN[g_tm.tm_wday]);
//    EPD_DrawUTF8(10, 260, 1, temp_buf, EPD_ASCII_11X16, NULL, BLACK, WHITE);
//}
...
//    // 2. 更新当前的 Unix 时间戳
//    current_unix_time += time_offset;
//    time_offset = 60; // 之后每次都加 60 秒
//    
//    // 3. 【核心】如果当前没有正在进行的显示任务，就启动一次新的显示
//    if (step == 0)
//    {
//        step = 1;
//        display();
...
//    EPD_DrawUTF8(30, 50, 2, "日一二三四五六", EPD_ASCII_11X16, EPD_FontUTF8_24x24, BLACK, FONT_BACKGROUND);

//Paint_DrawRectangle(10, 10, 390, 290, BLACK, DOT_PIXEL_2X2, DRAW_FILL_EMPTY);

    // 2. 在矩形内部，再画一个小一点的、填充的矩形，以验证填充功能
//    Paint_DrawRectangle(100, 100, 150, 200, BLACK, DOT_PIXEL_1X1, DRAW_FILL_FULL);

    // 3. 在另一个角落，画一条线，验证画线功能
//    Paint_DrawLine(380, 280, 200, 200, BLACK, DOT_PIXEL_2X2, LINE_STYLE_SOLID);
