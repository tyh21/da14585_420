                                        ...
                                        ke_task_id_t const src_id)
{
    ...
    if (bat_lvl_alert_used)
    {
        if (bat_led_state)
        {
            ...
            bat_led_state = 0;
            app_timer_set(APP_BASS_ALERT_TIMER, dest_id, 10);
        }
        else
        {
            GPIO_SetActive(bat_led_port, bat_led_pin);
            bat_led_state = 1;
            app_timer_set(APP_BASS_ALERT_TIMER, dest_id, 500);
        }
    }
    return (KE_MSG_CONSUMED);
}
 ...
 ****************************************************************************************
 */
static int app_bass_connect_timer_handler(ke_msg_id_t const msgid,
                                        void const *param,
                                        ke_task_id_t const dest_id,
                                        ke_task_id_t const src_id)
{
    ...
    if (bat_connect_used)
    {
        if (bat_connect_led_state)
        {
            GPIO_SetInactive(GPIO_LED_PORT, GPIO_LED_G_PIN);
            bat_connect_led_state = 0;
            app_timer_set(APP_BASS_CONNECT_TIMER, dest_id, 10);
        }
        else
        {
            GPIO_SetActive(GPIO_LED_PORT, GPIO_LED_G_PIN);
            bat_connect_led_state = 1;
            app_timer_set(APP_BASS_CONNECT_TIMER, dest_id, 300);
        }
    }else{
        GPIO_SetActive(GPIO_LED_PORT, GPIO_LED_G_PIN);
        ke_timer_clear(APP_BASS_CONNECT_TIMER, TASK_APP);
    }
    return (KE_MSG_CONSUMED);
}
...
static const struct ke_msg_handler app_bass_process_handlers[] =
{
    ...
    {APP_BASS_CONNECT_TIMER,                (ke_msg_func_t)app_bass_connect_timer_handler},
};
