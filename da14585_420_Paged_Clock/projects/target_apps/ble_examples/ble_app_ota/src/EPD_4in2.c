...
{
    // 步骤 1: 发送刷新指令序列
    EPD_4IN2_V2SendCommand(0x22); // CMD_DISP_CTRL2
    EPD_4IN2_V2SendData(0xF7);    // 使用0xF7，这是更完整的刷新参数
    EPD_4IN2_V2SendCommand(0x20); // CMD_MASTER_ACTIVATE
    // 步骤 2: 【关键】长时间等待刷新完成
    // 这里的等待是导致蓝牙断开的直接原因！
    EPD_4IN2_V2ReadBusy(); // 必须等待，否则刷新无法完成
    // 步骤 3: （可选但推荐）刷新后关闭电源
    // EPD_4IN2_V2SendCommand(0x22);
    // EPD_4IN2_V2SendData(0x83); // 对应 Update(0x83) power off
...
{
    // EPD_4IN2_V2SendCommand(0x22); //POWER OFF
    // EPD_4IN2_V2SendData(0xC3);
    // EPD_4IN2_V2SendCommand(0x20);

    EPD_4IN2_V2SendCommand(0x10); //enter deep sleep
    EPD_4IN2_V2SendData(0x01);
    // DEV_Delay_ms(100);
}
void EPD_Init_Pure_Weixue_Sequence(void)
{
    ...
    EPD_4IN2_V2SendCommand(0x21); // Display update control
    ...
    // Set RAM X-address Start/End position (0 to 399)
    EPD_4IN2_V2SendCommand(0x44);
    EPD_4IN2_V2SendData(0);           // X-start = 0
    EPD_4IN2_V2SendData(49);          // X-end = 49 (399 / 8)

    // Set RAM Y-address Start/End position (0 to 299)
    EPD_4IN2_V2SendCommand(0x45);
    EPD_4IN2_V2SendData(0);           // Y-start low = 0
    EPD_4IN2_V2SendData(0);           // Y-start high = 0
    EPD_4IN2_V2SendData(299 % 256);   // Y-end low = 43
    EPD_4IN2_V2SendData(299 / 256);   // Y-end high = 1
