...
uint8_t bat_connect_used      __SECTION_ZERO("retention_mem_area0"); //@RETENTION MEMORY
uint8_t bat_connect_led_state __SECTION_ZERO("retention_mem_area0"); //@RETENTION MEMORY
...
void app_batt_lvl(void)
{
    ...
    {
      if (cur_batt_level < BATTERY_ALERT_AT_PERCENTAGE_LEFT)
      {
          app_batt_alert_start(); // Start battery level alert
      }

      if (cur_batt_level > BATTERY_ALERT_AT_PERCENTAGE_LEFT + 5)
      {
    ...
    if(bat_lvl_alert_used && cur_batt_level < 5)
    {
...
void app_batt_alert_start(void)
{
    ...
    bat_led_state = 1;
    app_timer_set(APP_BASS_ALERT_TIMER, TASK_APP, 500);
#else
    if (bat_lvl_alert_used)
    {
