...
void EPD_2IN13_V2_TurnOnDisplay_Partial(void)
{
    EPD_2IN13_V2_SendCommand(0x22);
    EPD_2IN13_V2_SendData(0xFF); // <-- The magic number for Partial Update!
    EPD_2IN13_V2_SendCommand(0x20);
    EPD_2IN13_V2_ReadBusy(); // Partial update still needs waiting.
}
...
// 在您的 EPD_2in13_V2.c 中

/******************************************************************************
* @brief  Initializes the e-Paper register based on the pure command sequence
*         extracted from the working Weixue 4.2inch V2 driver.
* @note   This function uses our own, verified low-level functions.
******************************************************************************/
