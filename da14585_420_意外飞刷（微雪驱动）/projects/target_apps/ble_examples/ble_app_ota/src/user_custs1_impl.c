...
{
    // 这个函数使用备份的、上一分钟的字符串
    EPD_DrawUTF8(10, 10, 1, old_date_str, EPD_ASCII_11X16, NULL, BLACK, WHITE);
...
{
    // ================== 【关键新增】备份旧字符串 ==================
    // 在计算任何新时间之前，将当前的字符串（它们是上一分钟的）
    // 复制到备份变量中。
    strcpy(old_date_str, date_str);
    strcpy(old_time_str, time_str);
    strcpy(old_lunar_str, lunar_str);
		// 1. 设置下一次的 60 秒定时器
...
void display(void)
{
    ...
    switch (step)
    {
        ...
        case 1: // 初始化
            if (is_full_update) {
            //EPD_2IN13_V2_Init(0); // 执行我们已有的、正常工作的全刷 Init
            EPD_Init_Pure_Weixue_Sequence(); 
							partial_mode_initialized = false; // 全刷后，局刷模式失效，需要重新初始化
        } else {
            // 如果将要进行局刷，并且局刷模式还未被初始化
            if (!partial_mode_initialized) {
                EPD_2IN13_V2_Init_Partial_Mode_Legacy();  // 执行新的局刷模式初始化
                partial_mode_initialized = true; // 标记一下，避免重复初始化
            }
        }
        page_to_render = 0;
        step++;
        ...
        if (is_full_update) {
            // 【全刷模式】
            // 我们需要一个空白的红色通道。
            // Paint_Clear(WHITE) 已经把缓冲区填满了 0xFF，
            // 这通常就代表“无红色”，所以我们什么都不用再画了。
        } else {
            // 【局刷模式】
            // 在缓冲区里绘制上一分钟的画面。
        ...
        if (is_full_update) {
            ...
            EPD_2IN13_V2_TurnOnDisplay_Partial_Legacy();  // 局刷 (0xC7)
        }
        step++;
        break;
